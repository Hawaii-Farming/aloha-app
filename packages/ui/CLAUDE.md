...
- Use `cn()` utility from `@aloha/ui/utils` for class merging
...
Use the `If` component from `packages/ui/src/kit/if.tsx`:
...
Always use `Trans` component from `packages/ui/src/kit/trans.tsx`:
...
Use the `toast` utility from `@aloha/ui/sonner`:
...
```tsx
import { Spinner } from '@aloha/ui/spinner';
...
```tsx
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@aloha/ui/card';
...
import { FormField, FormItem, FormLabel, FormControl, FormMessage } from '@aloha/ui/form';
