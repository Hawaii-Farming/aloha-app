...
export class ComponentsTool {
  ...
  private static determineCategory(
    ...
    if (filePath.includes('/kit/')) return 'kit';
    return 'utils';
  }
  ...
  private static async generateDescription(
    ...
    if (category === 'shadcn') {
      ...
      return this.getAlohaDescription(componentName);
    } else {
  ...
  private static getAlohaDescription(componentName: string): string {
    const descriptions: Record<string, string> = {
    ...
    return (
      descriptions[componentName] || `Aloha custom component: ${componentName}`
    );
  }
...
function createGetComponentsTool(server: McpServer) {
  return server.tool(
    'get_components',
    'Get all available UI components from the @aloha/ui package with descriptions',
    async () => {
