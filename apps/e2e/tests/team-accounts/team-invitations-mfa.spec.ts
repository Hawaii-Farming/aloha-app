...
test.describe('Team Invitation with MFA Flow', () => {
  test('complete flow: test@example.com creates team, invites super-admin@example.com who accepts after MFA', async ({
    page,
  }) => {
    ...
    // Step 1: test@example.com creates a team and sends invitation
    ...
    // Sign out test@example.com
    await auth.signOut();
    await page.waitForURL('/');

    // Step 2: super-admin@example.com signs in with MFA
    await page.context().clearCookies();

    await auth.visitConfirmEmailLink('super-admin@example.com');
