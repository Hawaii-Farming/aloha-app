...
// check requirements to run the template
...
/**
 * Checks if the current pnpm version is compatible with the template.
...
function checkPnpmVersion() {
  ...
  if (!currentPnpmVersion) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
      `You are running the template from a directory that does not have pnpm installed. Please install pnpm and run "pnpm install" in your project directory.`,
    );
  ...
  if (major < 9) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
      `You are running pnpm ${currentPnpmVersion}. the template requires pnpm ${requiredPnpmVersion}.`,
    );
  ...
  if (major < 9 && minor < 12) {
    console.warn(
      `\x1b[33m%s\x1b[0m`,
      `You are running pnpm ${currentPnpmVersion}. the template recommends using pnpm 9.12.0 or higher.`,
    );
  } else {
    console.log(
      `\x1b[32m%s\x1b[0m`,
...
/**
 * Checks if the current Node version is compatible with the template.
...
function checkNodeVersion() {
  ...
  if (major < 18 || (major === 18 && minor < 18)) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
      `You are running Node ${currentNodeVersion}. the template requires Node ${requiredNodeVersion}.`,
    );
...
function checkPathNotOneDrive() {
  ...
  if (path.includes('OneDrive')) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
      `You are running the template from OneDrive. Please move your project to a local folder.`,
    );
