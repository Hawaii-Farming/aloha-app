...
function checktemplateVersion() {
  ...
  if (fetchResult === null) {
    console.info(
      '\x1b[33m%s\x1b[0m',
      "⚠️ You have not setup 'upstream'. Please set up the upstream remote so you can update your template version.",
    );
  ...
  if (severity === 'critical') {
    ...
    console.log(
      '\x1b[31m%s\x1b[0m',
      '❌  Your template version is outdated. Please update to the latest version.',
    );
  } else if (severity === 'warning') {
    console.log(
      '\x1b[33m%s\x1b[0m',
      '⚠️  Your template version is outdated! Best to update to the latest version.',
    );
  } else {
    console.log('\x1b[32m%s\x1b[0m', '✅ Your template version is up to date!');
  }
