...
- **unit_of_measure** — Standardized measurement units with `code` as primary key (kg, L, °C, ppm, etc.)
...
- **fob** — Org-specific FOB (Free On Board) delivery points
- **sales_cust** — Org customers with group, FOB preference, billing, and external accounting link
- **invnt_vendor** — Org-level vendors for procurement with payment terms and lead time
...
- **inventory_item** — Items (seed, chemical, packaging, part, supply) with unit conversions, burn rates, reorder settings
...
- **sales_product_inventory_item** — Links sales products to inventory items at pack and sale levels for inventory tracking
...
- **hr_task** — Hierarchical task catalog with self-referencing parent for unlimited nesting
...
- **`unit_of_measure.code`** (VARCHAR) as PK — all unit FK columns across the system are `VARCHAR(10)` referencing `unit_of_measure(code)`
...
- **Module prefixes** for table naming: `sales_*`, `invnt_*`, `grow_*`, `hr_*`
