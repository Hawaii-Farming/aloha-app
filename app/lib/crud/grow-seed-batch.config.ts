...
const growSeedBatchSchema = z.object({
  ...
  estimated_harvest_date: z.string().min(1, 'Estimated harvest date is required'),
  status: z
...
export const growSeedBatchConfig: CrudModuleConfig<typeof growSeedBatchSchema> = {
  ...
  filters: [
    {
      ...
      label: 'Status',
      type: 'select',
      options: ['planned', 'seeded', 'transplanted', 'harvesting', 'harvested'],
    },
  ],
  ...
  formFields: [
    ...
    { key: 'number_of_units', label: 'Number of Units', type: 'number', required: true },
    { key: 'seeds_per_unit', label: 'Seeds per Unit', type: 'number', required: true },
    { key: 'number_of_rows', label: 'Number of Rows', type: 'number', required: true },
    { key: 'seeding_date', label: 'Seeding Date', type: 'date', required: true },
