...
import { GenerateInvoiceSchema } from '../_lib/schemas/invoice.schema';
import { createInvoiceService } from '../_lib/server/invoice.service';
...
export async function action({ request }: { request: Request }) {
  ...
  const result = await service.generatePdf(parsed.data.invoiceId);
...
import { OnboardAccountSchema } from '../_lib/schemas/onboarding.schema';
import { createOnboardingService } from '../_lib/server/onboarding.service';
import { createProjectService } from '../_lib/server/project.service';
import { createNotificationService } from '../_lib/server/notification.service';

export async function action({ request }: { request: Request }) {
  const formData = await request.json();
  const parsed = OnboardAccountSchema.safeParse(formData);
  ...
  const result = await service.onboardAccount(parsed.data);
...
export function calculatePricing(input: PricingInput): PricingResult {
  ...
  const yearlyDiscount = input.period === 'yearly' ? 0.2 : 0;
