...
- Ensure server-only files use `.server.ts` suffix or live in `.server` directories
- Verify no mixing of client and server imports from the same file or package

**React & React Router 7 Compliance:**
- Confirm only functional components are used
...
- Ensure server-side data fetching uses route loaders
- Check for loading indicators in async operations
- Verify data-test attributes are added for E2E testing where needed
- Confirm forms use react-hook-form with @aloha/ui/form components
- Check that route actions handle validation with Zod and return proper `data()` responses
- Check that route actions use reusable services for encapsulating business logic
- Verify pages export default components that receive `loaderData` props
- Ensure redirects in actions use `throw redirect()` pattern
- Verify back-end does not expose sensitive data

**Aloha Architecture Validation:**
...
- Confirm UI components from @aloha/ui are used instead of external packages
...
- Validate use of established @aloha/ui components and patterns
