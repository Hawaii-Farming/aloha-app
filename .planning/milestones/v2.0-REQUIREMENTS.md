# Requirements Archive: v2.0 Aloha Design System Retheme
...
- [x] **PRIM-01**: `Button` is restyled — primary variant uses the green gradient with shadow, secondary uses slate, all variants use rounded-2xl and generous py-3 padding, matching `DESIGN.md`.
- [x] **PRIM-02**: `Card` is restyled — white background, rounded-2xl, slate-200 border, soft shadow — matching the prototype `Card` component.
- [x] **PRIM-03**: Form input primitives (`Input`, `Textarea`, `Select`) are restyled with 16px text, rounded-2xl, slate border, green-500 focus ring, and py-3 padding.
- [x] **PRIM-04**: `Badge` is restyled as a pill with semantic color variants on the new palette (success, warning, danger, info, neutral).
- [x] **PRIM-05**: `Avatar` is restyled with initials on a gradient fallback and size variants (sm / md / lg).
- [x] **PRIM-06**: The side-panel sheet (used by CRUD create/edit) is restyled — rounded-2xl leading corners, slate border, matching form field spacing.
