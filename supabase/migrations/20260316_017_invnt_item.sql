CREATE TABLE IF NOT EXISTS invnt_item (
    ...
    order_uom                VARCHAR(10) REFERENCES util_uom(code),
    onhand_burn_quantity     NUMERIC,
    order_burn_quantity      NUMERIC,
    ...
    -- Site references
    org_site_id_storage      UUID REFERENCES org_site(id),
    org_site_id_equipment    UUID REFERENCES org_site(id),
...
CREATE INDEX idx_invnt_item_site_storage ON invnt_item (org_site_id_storage);
CREATE INDEX idx_invnt_item_site_equipment ON invnt_item (org_site_id_equipment);
...
COMMENT ON COLUMN invnt_item.external_id IS 'External system identifier for integration';
...
COMMENT ON COLUMN invnt_item.onhand_burn_quantity IS 'Number of burn units in one onhand unit';
COMMENT ON COLUMN invnt_item.order_burn_quantity IS 'Number of burn units in one order unit';
...
COMMENT ON COLUMN invnt_item.org_site_id_storage IS 'Storage site where this item is kept; references org_site';
COMMENT ON COLUMN invnt_item.org_site_id_equipment IS 'Equipment site this part belongs to; references org_site';
...
COMMENT ON COLUMN invnt_item.created_by IS 'User who created the record, references auth.users(id)';
COMMENT ON COLUMN invnt_item.updated_at IS 'Timestamp when the record was last updated';
COMMENT ON COLUMN invnt_item.updated_by IS 'User who last updated the record, references auth.users(id)';
