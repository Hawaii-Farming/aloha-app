...
COMMENT ON COLUMN sales_cust.external_id IS 'External accounting system identifier for integration';
...
COMMENT ON COLUMN sales_cust.created_by IS 'User who created the record, references auth.users(id)';
COMMENT ON COLUMN sales_cust.updated_at IS 'Timestamp when the record was last updated';
COMMENT ON COLUMN sales_cust.updated_by IS 'User who last updated the record, references auth.users(id)';
