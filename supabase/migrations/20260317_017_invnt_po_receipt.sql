CREATE TABLE IF NOT EXISTS invnt_po_receipt (
    ...
    receipt_uom            TEXT REFERENCES util_uom(code),
    receipt_quantity       NUMERIC NOT NULL,
    burn_per_receipt_uom  NUMERIC,
...
COMMENT ON COLUMN invnt_po_receipt.burn_per_receipt_uom IS 'Conversion factor: burn units per receipt unit at time of receipt';
...
COMMENT ON COLUMN invnt_po_receipt.created_by IS 'Email of the user who created the record';
COMMENT ON COLUMN invnt_po_receipt.updated_at IS 'Timestamp when the record was last updated';
COMMENT ON COLUMN invnt_po_receipt.updated_by IS 'Email of the user who last updated the record';
