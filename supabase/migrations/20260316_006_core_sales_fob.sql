...
COMMENT ON COLUMN sales_fob.created_by IS 'User who created the record, references auth.users(id)';
COMMENT ON COLUMN sales_fob.updated_at IS 'Timestamp when the record was last updated';
COMMENT ON COLUMN sales_fob.updated_by IS 'User who last updated the record, references auth.users(id)';
