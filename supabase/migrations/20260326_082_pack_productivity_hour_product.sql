...
COMMENT ON TABLE pack_productivity_hour_product IS 'Cases packed per product per hour (delta, not cumulative). Leftover pounds tracked per product. Derived metrics: trays = cases_packed × sales_product.pack_per_case, pounds = cases_packed × sales_product.case_net_weight.';
...
COMMENT ON COLUMN pack_productivity_hour_product.leftover_pounds IS 'Leftover product weight for this product at end of this hour; shift total is SUM across products';
