CREATE TABLE IF NOT EXISTS ops_corrective_action_taken (
    ...
    fsafe_result_id                 UUID,       -- FK to fsafe_result(id) added via ALTER TABLE in fsafe_result migration
...
COMMENT ON COLUMN ops_corrective_action_taken.fsafe_result_id IS 'Null if triggered by a checklist response; FK added via ALTER TABLE in fsafe_result migration';
