...
COMMENT ON COLUMN invnt_sales_product_item.quantity_per_unit IS 'Quantity of the inventory item consumed per unit at the specified packaging level';
...
COMMENT ON COLUMN invnt_sales_product_item.created_by IS 'User who created the record, references auth.users(id)';
COMMENT ON COLUMN invnt_sales_product_item.updated_at IS 'Timestamp when the record was last updated';
COMMENT ON COLUMN invnt_sales_product_item.updated_by IS 'User who last updated the record, references auth.users(id)';
