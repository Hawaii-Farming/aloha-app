CREATE TABLE IF NOT EXISTS invnt_po (
    ...
    request_type           VARCHAR(20) NOT NULL DEFAULT 'inventory_item' CHECK (request_type IN ('general', 'inventory_item')),
    urgency_level          TEXT,
...
COMMENT ON COLUMN invnt_po.request_type IS 'Whether this is a general purchase or an inventory item purchase: general, inventory_item';
COMMENT ON COLUMN invnt_po.urgency_level IS 'Number of days until the item is needed (e.g. 1, 2, 7)';
COMMENT ON COLUMN invnt_po.category_id IS 'Category for general requests (provides classification when there is no linked inventory item)';
COMMENT ON COLUMN invnt_po.invnt_item_id IS 'Linked inventory item; NULL for general requests';
COMMENT ON COLUMN invnt_po.item_name IS 'Snapshot of item name at order time; manually entered for general requests';
...
COMMENT ON COLUMN invnt_po.order_burn_quantity IS 'Snapshot of burn units per order unit at order time';
...
COMMENT ON COLUMN invnt_po.external_id IS 'External PO number or vendor reference';
...
COMMENT ON COLUMN invnt_po.updated_by IS 'User who last updated the record, references auth.users(id)';
