...
COMMENT ON TABLE ops_template_result IS 'Employee responses to checklist questions. One row per question per task tracker session. The linked ops_task_tracker record acts as the header (who completed the checklist, when, and at which site).';
...
COMMENT ON COLUMN ops_template_result.site_id IS 'Sourced from fsafe_lab_test.atp_site_count random selection of zone_1 sites; null for standard checklist responses';
