...
erDiagram
    ...
    site ||--o{ hr_task_site : location
    hr_task_tracker ||--o{ hr_task_site : has
    ...
    org ||--o{ hr_training : has
    farm ||--o{ hr_training : scoped
    hr_employee ||--o{ hr_training : verifies
    hr_training ||--o{ hr_training_attendee : has
    hr_employee ||--o{ hr_training_attendee : attends
```
...
| hr_task_site | Sites where a task event was performed. One row per site per task tracker; supports tasks across multiple sites. |
...
| hr_training | Staff training session records. Each row is one training event covering a specific topic for a group of employees. |
| hr_training_attendee | Individual attendance and certification records for each employee per training session. One row per employee per training. |
...
| training_type           | TEXT         | nullable                       | Category of training conducted (e.g. GMP, Food Safety, Sanitation, Equipment Operation, GAPs, HACCP) |
| training_date           | DATE         | nullable                       | Date the training was conducted          |
| topics_covered          | TEXT         | nullable                       | Description of the topics covered during the training session |
| trainer_name            | TEXT         | nullable                       | Name of the trainer; may be an external trainer or internal employee name |
| materials_url           | TEXT         | nullable                       | URL or path to the training materials or presentation used |
| certification_issued    | BOOLEAN      | NOT NULL, default false        | Whether a certification was issued to attendees upon completion |
| certification_valid_days | INTEGER     | nullable                       | Number of days the certification is valid from the date of issue; null if no expiry |
...
| org_id                  | TEXT         | NOT NULL, FK → org(id)                | Owning organization for RLS filtering    |
| training_id             | UUID         | NOT NULL, FK → hr_training(id)        | Training session this attendance record belongs to |
| employee_id             | TEXT         | NOT NULL, FK → hr_employee(id)        | Employee who attended or was scheduled to attend the training |
| attended_training       | BOOLEAN      | NOT NULL, default false               | Whether the employee attended the training session |
...
Unique constraint on `(training_id, employee_id)`.
