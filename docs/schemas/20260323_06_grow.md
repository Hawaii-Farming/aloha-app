...
erDiagram
    ...
    org_farm ||--o{ grow_seeding_batch : seeds
    ops_task_tracker ||--o{ grow_seeding_batch : tracks
    grow_trial_type ||--o{ grow_seeding_batch : classifies
    grow_seed_mix ||--o{ grow_seeding_batch : blends
    invnt_item ||--o{ grow_seeding_batch : uses
```
...
| grow_seeding_batch | Individual seeding batch linked to an ops activity. Either a single seed item or a seed mix, never both. |
...
| invnt_item_id | UUID | NOT NULL, FK → invnt_item(id) | |
...
| invnt_item_id | UUID | FK → invnt_item(id), nullable | Set if seeding a single variety; null for mixes |
