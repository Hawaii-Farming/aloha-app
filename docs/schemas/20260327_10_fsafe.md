...
erDiagram
    ...
    ops_task_tracker ||--o{ fsafe_pest_trap_inspection : inspects
    org_site ||--o{ fsafe_pest_trap_inspection : station
    org_farm ||--o{ fsafe_pest_trap_inspection : scoped
```
...
| fsafe_pest_trap_inspection | Per-station pest trap inspection result. One row per trap station per inspection event. The ops_task_tracker acts as the inspection header with date, farm, and verification. |
...
| pest_type | TEXT | nullable, CHECK | mouse, rat, cockroach, ant, other; null means no activity at this station |
