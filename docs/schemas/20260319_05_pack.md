...
erDiagram
    ...
    farm ||--o{ pack_lot : scoped
    pack_lot ||--o{ pack_lot_item : contains
    sales_product ||--o{ pack_lot_item : packed_in
    util_uom ||--o{ pack_lot_item : measures

    farm ||--o{ pack_packaging_type : scoped
    farm ||--o{ pack_shelf_life_check : scoped
    farm ||--o{ pack_shelf_life_trial : scoped
...
| farm_id | TEXT | NOT NULL, FK → farm(id) | Farm (crop line) this pack lot belongs to |
...
| org_id | TEXT | NOT NULL, FK → org(id) | Owning organization for RLS filtering |
| farm_id | TEXT | NOT NULL, FK → farm(id) | Farm (crop line) this pack lot item belongs to; inherited from parent pack_lot |
...
| uom | TEXT | NOT NULL, FK → util_uom(code) | Unit of measure for quantity packed; defaults to the product sale unit (e.g. case) |
| quantity_packed | NUMERIC | NOT NULL | Number of units packed for this product in this lot |
...
| org_id | TEXT | NOT NULL, FK → org(id) | Owning organization for RLS filtering |
| farm_id | TEXT | FK → farm(id), nullable | Optional farm scope; null if the packaging type applies to all farms |
| name | TEXT | NOT NULL | Packaging type name, unique per org and farm scope (e.g. Clamshell, Bag, Sleeve, Tray Wrap) |
| description | TEXT | nullable | Optional description of this packaging type |
...
| id | TEXT | PK | Human-readable identifier derived from name (trimmed lowercase) |
| org_id | TEXT | NOT NULL, FK → org(id) | Owning organization for RLS filtering |
| farm_id | TEXT | FK → farm(id), nullable | Optional farm scope; null if the check applies to all farms |
| name | TEXT | NOT NULL | Check name, unique per org and farm scope (e.g. Color, Texture, Moisture, Physical Damage) |
| description | TEXT | nullable | Optional description of what to look for during this check |
...
| org_id | TEXT | NOT NULL, FK → org(id) | Owning organization for RLS filtering |
| farm_id | TEXT | FK → farm(id), nullable | Optional farm scope; null if the trial applies to all farms |
| pack_lot_id | UUID | FK → pack_lot(id), nullable | Lot being tested; null if the trial is not tied to a specific lot |
| sales_product_id | TEXT | NOT NULL, FK → sales_product(id) | Product being tested in this trial |
| pack_packaging_type_id | TEXT | FK → pack_packaging_type(id), nullable | Packaging type used for this trial; null if same as the product default |
...
| notes | TEXT | nullable | Free-text notes about the trial |
...
| org_id | TEXT | NOT NULL, FK → org(id) | Owning organization for RLS filtering |
| farm_id | TEXT | FK → farm(id), nullable | Optional farm scope; inherited from parent pack_shelf_life_trial |
...
| response_boolean | BOOLEAN | nullable | Boolean response value; used when check response_type is boolean |
| response_numeric | NUMERIC | nullable | Numeric response value; used when check response_type is numeric |
| response_enum | TEXT | nullable | Selected enum option; used when check response_type is enum |
| response_text | TEXT | nullable | Free-text response; used when check response_type is text |
| notes | TEXT | nullable | Free-text notes about this observation |
...
| caption | TEXT | nullable | Optional caption describing what the photo shows |
