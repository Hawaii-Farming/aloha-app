...
| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| id | TEXT | PK | Same as sys_module_id for this org |
...
| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| id | TEXT | PK | Same as sys_sub_module_id for this org |
| org_id | TEXT | NOT NULL, FK → org(id) | |
| sys_module_id | TEXT | NOT NULL, FK → sys_module(id) | Parent module grouping |
| sys_sub_module_id | TEXT | NOT NULL, FK → sys_sub_module(id) | |
| sys_access_level_id | TEXT | NOT NULL, FK → sys_access_level(id) | Minimum access level required; inherited from system, adjustable per org |
| display_name | TEXT | NOT NULL | Customizable display name for this org |
| display_order | INTEGER | NOT NULL, default 0 | |
...
| org_id | TEXT | NOT NULL, FK → org(id) | |
| name | TEXT | NOT NULL | |
| weighing_uom | TEXT | FK → sys_uom(code), nullable | Default unit of measure for weighing operations on this farm (e.g. lb, kg) |
| growing_uom | TEXT | FK → sys_uom(code), nullable | Default unit of measure for growing/harvest tracking on this farm |
| volume_uom | TEXT | FK → sys_uom(code), nullable | Default unit of measure for water and liquid volumes on this farm (e.g. gallons, liters) |
