...
| minimum_rlu_value| NUMERIC      | nullable                                     | Minimum acceptable RLU value for ATP tests on this template; results below this are a fail |
| maximum_rlu_value| NUMERIC      | nullable                                     | Maximum acceptable RLU value for ATP tests on this template; results above this are a fail |
...
| site_id              | TEXT         | FK → org_site(id), nullable                  | Sourced from ops_template.atp_site_count random selection; null for standard checklist responses |
...
| org_id                             | TEXT         | NOT NULL, FK → org(id)                              | |
| farm_id                            | TEXT         | FK → org_farm(id), nullable                             | |
| ops_template_id                    | TEXT         | FK → ops_template(id), nullable                     | Inherited from ops_template_response.ops_template_id when sourced from a failing checklist response |
| ops_template_response_id                    | UUID         | FK → ops_template_response(id), nullable                     | Sourced from the failing ops_template_response that triggered this corrective action |
