...
erDiagram
    ...
    pack_shelf_life ||--o{ pack_shelf_life_observation : observed
    org_farm ||--o{ pack_shelf_life_observation : scoped
    pack_shelf_life_metric ||--o{ pack_shelf_life_observation : checks
    pack_shelf_life ||--o{ pack_shelf_life_photo : captured
    org_farm ||--o{ pack_shelf_life_photo : scoped
    org ||--o{ pack_fail_category : has
    ops_task_tracker ||--o{ pack_productivity_hour : tracked
    pack_productivity_hour ||--o{ pack_productivity_hour_product : contains
    pack_productivity_hour ||--o{ pack_productivity_hour_fail : logged
    sales_product ||--o{ pack_productivity_hour_product : packed
    pack_fail_category ||--o{ pack_productivity_hour_fail : categorizes
```
...
| pack_productivity_hour | Hourly pack line snapshot with crew counts by role and metal detection flag. Derived: total_trays, trays_per_packer_per_minute, packed_pounds. |
| pack_productivity_hour_product | Cases packed per product per hour (delta, not cumulative) with leftover pounds. |
...
Individual products packed within a lot. One row per product per lot. `pack_quantity` is always in the product's `sale_uom`.
...
| sales_product_id | TEXT | NOT NULL, FK → sales_product(id) | |
| invnt_item_id_pack | TEXT | FK → invnt_item(id), nullable | Pre-filled from sales_product.invnt_item_id_pack; filtered to packaging items in inventory |
...
| pack_hour | TIMESTAMPTZ | NOT NULL | The hour being recorded (e.g. 2026-03-26 11:00); one row per clock hour |
...
| is_metal_detected | BOOLEAN | NOT NULL, default false | Whether metal was detected during this hour |
| notes | TEXT | nullable | |

Unique constraint on `(ops_task_tracker_id, pack_hour)`.

---

## pack_productivity_hour_product

Cases packed per product per hour (delta, not cumulative). Leftover pounds tracked per product; shift total is SUM across products.
...
| sales_product_id | TEXT | NOT NULL, FK → sales_product(id) | |
| cases_packed | INTEGER | NOT NULL, default 0 | Number of cases packed THIS hour for this product (delta, not cumulative) |
| leftover_pounds | NUMERIC | NOT NULL, default 0 | Leftover product weight for this product at end of this hour; shift total is SUM across products |

Unique constraint on `(pack_productivity_hour_id, sales_product_id)`.
...
| farm_id | TEXT | NOT NULL, FK → org_farm(id) | |
| pack_productivity_hour_id | UUID | NOT NULL, FK → pack_productivity_hour(id) | |
| pack_fail_category_id | TEXT | NOT NULL, FK → pack_fail_category(id) | |
| fail_count | INTEGER | NOT NULL, default 0 | Number of fails for this category in this hour |
| notes | TEXT | nullable | |

Unique constraint on `(pack_productivity_hour_id, pack_fail_category_id)`.
