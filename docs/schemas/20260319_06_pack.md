...
> **Note:** The `pack_packaging_type_id` column is defined on `sales_product` in its own migration. This migration adds the FK constraint via ALTER TABLE now that `pack_packaging_type` exists.
