...
erDiagram
    ...
    grow_fertigation_recipe ||--o{ grow_task_seed_batch : applied
...
| grow_task_seed_batch | Unified join table linking any grow activity (scouting, spraying, fertigation, monitoring) to seeding batches. Replaces the 4 individual seed batch tables. Nullable grow_fertigation_recipe_id for fertigation activities. |
...
Unified join table linking any grow activity (scouting, spraying, fertigation, monitoring) to one or more seeding batches. Replaces the 4 individual tables: `grow_scout_seed_batch`, `grow_spray_seed_batch`, `grow_fertigation_seed_batch`, and `grow_monitoring_seed_batch`. The nullable `grow_fertigation_recipe_id` is only set for fertigation activities to record which recipe was applied.
...
| grow_seed_batch_id | UUID | NOT NULL, FK → grow_seed_batch(id) | |
| grow_fertigation_recipe_id | TEXT | FK → grow_fertigation_recipe(id), nullable | Only set for fertigation activities; null for scouting, spraying, and monitoring |
