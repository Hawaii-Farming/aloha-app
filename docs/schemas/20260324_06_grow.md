...
erDiagram
    ...
    org_farm ||--o{ grow_monitoring_point : defines
    ops_task_tracker ||--o{ grow_monitoring_reading : records
    org_site ||--o{ grow_monitoring_reading : located
    grow_monitoring_point ||--o{ grow_monitoring_reading : measures
    ops_task_tracker ||--o{ grow_monitoring_seeding : snapshots
    grow_seeding ||--o{ grow_monitoring_seeding : present
    ops_task_tracker ||--o{ grow_monitoring_photo : documents
```
...
| grow_monitoring_point | Defines what to measure per farm + site category with UOM, thresholds, and optional formula. |
...
| grow_monitoring_photo | Photos taken during a monitoring event with optional caption. |
...
| phi_days | INTEGER | NOT NULL | Pre-Harvest Interval in days |
| rei_hours | INTEGER | NOT NULL | Restricted Entry Interval in hours |
| application_method | TEXT | NOT NULL | How the product is applied (e.g. spray, drench, granular) |
...
| application_per_burn_unit | NUMERIC | NOT NULL | Application rate expressed in burn units |
...
| farm_id | TEXT | NOT NULL, FK → org_farm(id) | |
| ops_task_tracker_id | UUID | NOT NULL, FK → ops_task_tracker(id) | |
| grow_spraying_compliance_id | UUID | NOT NULL, FK → grow_spraying_compliance(id) | Source of truth; invnt_item_id is pre-filled from this record |
| invnt_item_id | TEXT | NOT NULL, FK → invnt_item(id) | Pre-filled from the compliance record; not independently selected |
| target_pest_disease | JSONB | NOT NULL, default [] | Pests and diseases being targeted with this input |
...
| response_type | TEXT | NOT NULL, default 'numeric', CHECK | How the reading is captured: numeric, boolean, text |
| point_type | TEXT | NOT NULL, default 'direct', CHECK | direct = manually entered; calculated = derived from formula |
| formula | TEXT | nullable | Expression for calculated points (e.g. (drain_ml / (drip_ml * drippers)) * 100); null for direct |
| input_point_ids | JSONB | nullable | Array of grow_monitoring_point IDs that feed into this calculation; null for direct |
| corrective_actions | JSONB | NOT NULL, default [] | Available corrective action options for out-of-range readings |
...
| site_id | TEXT | NOT NULL, FK → org_site(id) | |
| ops_task_tracker_id | UUID | NOT NULL, FK → ops_task_tracker(id) | |
| grow_monitoring_point_id | TEXT | NOT NULL, FK → grow_monitoring_point(id) | |
| monitoring_station | TEXT | nullable | Selected from org_site.monitoring_stations JSONB dropdown |
| reading | NUMERIC | nullable | Used when point response_type is numeric |
...
| is_out_of_range | BOOLEAN | NOT NULL, default false | Auto-set by comparing reading against point thresholds |
| corrective_action | TEXT | nullable | Selected from grow_monitoring_point.corrective_actions when reading is out of range |
| notes | TEXT | nullable | |

Unique constraint on `(ops_task_tracker_id, grow_monitoring_point_id, monitoring_station)`.

---

## grow_monitoring_seeding

Point-in-time snapshot of which seedings were present during a monitoring event. Linked to ops_task_tracker.
...
## grow_monitoring_photo

Photos taken during a monitoring event. One row per photo with optional caption.
