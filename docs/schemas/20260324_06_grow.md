...
erDiagram
    ...
    ops_task_tracker ||--o{ grow_scout_seed_batch : covers
    grow_seed_batch ||--o{ grow_scout_seed_batch : inspected
    ...
    ops_task_tracker ||--o{ grow_scout_photo : documents
    invnt_item ||--o{ grow_spray_compliance : regulated
    ops_task_tracker ||--o{ grow_spray_seed_batch : covers
    grow_seed_batch ||--o{ grow_spray_seed_batch : treated
    ...
    ops_task_tracker ||--o{ grow_fertigation_seed_batch : fertigates
    grow_fertigation_recipe ||--o{ grow_fertigation_seed_batch : applied
    grow_seed_batch ||--o{ grow_fertigation_seed_batch : treated
    ...
    ops_task_tracker ||--o{ grow_monitoring_seed_batch : snapshots
    grow_seed_batch ||--o{ grow_monitoring_seed_batch : present
...
| grow_cycle_pattern | Defines growing cycle patterns per farm with days to transplant, harvest, and total cycle. |
...
| grow_scout_seed_batch | Join table linking a scouting activity to one or more seeding batches. |
...
| grow_scout_photo | Photos taken during a scouting event. One row per photo with optional caption. |
| grow_spray_compliance | Chemical label registry with REI, PHI, application rates, and regulatory info per product. |
| grow_spray_seed_batch | Join table linking spraying activities to seeding batches. |
...
| grow_fertigation_seed_batch | Snapshot of seedings fertigated per event + recipe link. |
...
| grow_cycle_pattern_id | TEXT | FK → grow_cycle_pattern(id), nullable | Used to auto-calculate transplant and harvest dates |
...
Defines growing cycle patterns per farm (e.g. 14-Day Lettuce, 42-Day Cucumber). Used to auto-calculate transplant and harvest dates on seeding batches.
...
| name | TEXT | NOT NULL | |
| description | TEXT | nullable | |
| days_to_transplant | INTEGER | nullable | Days from seeding date to transplant date |
| days_to_harvest | INTEGER | nullable | Days from transplant date to estimated harvest date |
| total_cycle_days | INTEGER | nullable | Total days from seeding to harvest |
