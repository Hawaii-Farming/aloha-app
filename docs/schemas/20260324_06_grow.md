...
| farm_id | TEXT | NOT NULL, FK → org_farm(id) | |
| ops_task_tracker_id | UUID | NOT NULL, FK → ops_task_tracker(id) | |
| grow_spraying_compliance_id | UUID | NOT NULL, FK → grow_spraying_compliance(id) | Compliance record — source of truth; invnt_item_id is pre-filled from this record |
...
| input_point_ids | JSONB | nullable | JSON array of grow_monitoring_point IDs that feed into this calculation; null for direct points |
...
| is_out_of_range | BOOLEAN | NOT NULL, default false | Auto-set by comparing reading against grow_monitoring_point.minimum_value and maximum_value |
