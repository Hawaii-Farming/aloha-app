...
erDiagram
    ...
    sys_pest ||--o{ grow_scout_observation : identified
    sys_disease ||--o{ grow_scout_observation : identified
...
| sys_pest_id | TEXT | FK → sys_pest(id), nullable | Shown when observation_type is pest; null when disease |
| sys_disease_id | TEXT | FK → sys_disease(id), nullable | Shown when observation_type is disease; null when pest |
...
CHECK constraint: `observation_type = pest` requires `sys_pest_id` set and `sys_disease_id` null, and vice versa.
...
| row_number | INTEGER | NOT NULL | The specific growing row within the site where this pest/disease was observed |
...
| equipment_id | TEXT | NOT NULL, FK → org_equipment(id) | Pre-filled from grow_fertigation_recipe_item.equipment_id for the selected recipe; editable |
...
| minimum_value | NUMERIC | nullable | Reading below this value auto-sets grow_monitoring_reading.is_out_of_range to true; null if not numeric |
| maximum_value | NUMERIC | nullable | Reading above this value auto-sets grow_monitoring_reading.is_out_of_range to true; null if not numeric |
...
| corrective_actions | JSONB | NOT NULL, default [] | JSON array of corrective action options shown when reading is out of range; selected value stored in grow_monitoring_reading.corrective_action |
