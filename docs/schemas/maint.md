...
> **Standard audit fields:** Every table includes `updated_at` (TIMESTAMPTZ, default now) and `updated_by` (TEXT, user email). These are omitted from the column listings below for brevity. `maint_request` uses `requested_at`/`requested_by` in place of `created_at`/`created_by`.
