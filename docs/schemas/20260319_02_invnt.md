...
erDiagram
    ...
    util_uom ||--o{ invnt_item : burn_unit
    util_uom ||--o{ invnt_item : onhand_unit
    util_uom ||--o{ invnt_item : order_unit
    org ||--o{ invnt_po : has
...
| maint_site_id_equipment  | TEXT         | FK → site(id), nullable               | Equipment site this part belongs to |
...
| seed_variety_id          | TEXT         | FK → grow_variety(id), nullable       | Linked crop variety for seed items       |
...
Tracks purchase order requests through a workflow from request to receipt. Each order snapshots the item name, units, and cost at order time so the record stays accurate even if the item changes later. `request_type` determines whether it's a `non_inventory_item` purchase (classified by `invnt_category_id`) or an `inventory_item` purchase (linked via `invnt_item_id`). `item_name` is always populated either way. Uses `requested_at`/`requested_by` in place of the standard `created_at`/`created_by` audit fields.
...
| org_id               | TEXT         | NOT NULL, FK → org(id)                | Owning organization for RLS filtering    |
| farm_id              | TEXT         | FK → farm(id), nullable               | Optional farm scope; inherited from parent invnt_po |
...
| received_uom         | TEXT         | FK → util_uom(code), nullable         | Unit of measure for the received quantity |
