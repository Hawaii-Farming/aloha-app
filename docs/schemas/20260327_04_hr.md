...
| site_id_housing          | TEXT         | FK → org_site(id), nullable                  | Sourced from org_site where category is housing; links employee to their assigned housing site |
...
| is_enabled | BOOLEAN | NOT NULL, default true | Pre-filled from org_module.is_enabled when employee access is provisioned; editable per employee |
