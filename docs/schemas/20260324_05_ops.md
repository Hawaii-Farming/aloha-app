...
| ops_question_id      | UUID         | FK → ops_question(id), nullable          | Null for ATP surface test results which have no associated question |
| site_id              | TEXT         | FK → org_site(id), nullable                  | Site where the ATP test was conducted; null for standard checklist responses |
| response_boolean     | BOOLEAN      | nullable                                 | Used when question response_type is boolean |
| response_numeric     | NUMERIC      | nullable                                 | Used when question response_type is numeric |
| response_enum        | TEXT         | nullable                                 | Used when question response_type is enum |
...
| ops_template_id                    | TEXT         | FK → ops_template(id), nullable                     | Null when triggered by an EMP test result which has no associated template |
| ops_response_id                    | UUID         | FK → ops_response(id), nullable                     | Null if triggered by an EMP test result |
| fsafe_result_id                    | UUID         | FK → fsafe_result(id), nullable                     | Null if triggered by a checklist response; FK added via ALTER TABLE in fsafe_result migration |
| ops_corrective_action_choice_id    | TEXT         | FK → ops_corrective_action_choice(id), nullable     | Null if a custom description is provided instead |
| other_action                       | TEXT         | nullable                                            | Used when no predefined choice is selected |
