...
| fsafe_test_hold_id          | UUID         | FK → fsafe_test_hold(id), nullable    | Parent test-and-hold record; null for standalone EMP results |
...
| result_pass                 | BOOLEAN      | nullable                              | Whether the result meets the pass criteria defined on the EMP test definition |
...
| fsafe_result_id_original    | UUID         | FK → fsafe_result(id), nullable       | Reference to the initial test result that triggered this retest or vector test; null for initial tests |
