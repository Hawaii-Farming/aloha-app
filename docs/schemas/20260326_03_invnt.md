...
| is_active      | BOOLEAN | NOT NULL, default true            | Auto-set to false when lot is fully consumed (received minus on-hand equals zero) |

Unique constraint on `(org_id, invnt_item_id, id)`.
...
| invnt_category_id    | TEXT         | NOT NULL, FK → invnt_category(id)     | Pre-filled from invnt_item.invnt_category_id when item is selected; editable |
| invnt_item_id        | TEXT         | FK → invnt_item(id), nullable         | |
| item_name            | TEXT         | NOT NULL                              | Snapshot from invnt_item.name at order time; preserved even if the item is later renamed |
| burn_uom             | TEXT         | NOT NULL, FK → sys_uom(code)         | Snapshot from invnt_item.burn_uom at order time |
| order_uom            | TEXT         | NOT NULL, FK → sys_uom(code)         | Snapshot from invnt_item.order_uom at order time |
| order_quantity       | NUMERIC      | NOT NULL                              | |
| burn_per_order       | NUMERIC      | NOT NULL, default 0                   | Snapshot from invnt_item.burn_per_order at order time |
...
| delivery_truck_clean | BOOLEAN      | nullable                              | |
| delivery_acceptable  | BOOLEAN      | nullable                              | |
| notes                | TEXT         | nullable                              | |
...
`invnt_usage` and `invnt_sales_product_item` are designed but deferred from the MVP. See [Future Improvements](20260325_11_future.md) for table definitions and planned features.
