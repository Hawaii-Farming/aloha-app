...
| `grow_spraying_seeding` | Join table — which seeding batches were treated |
| `grow_spraying_input` | Individual chemical/fertilizer applied with quantity and compliance link |
| `grow_spraying_equipment` | Equipment used with water UOM and quantity per piece |
| `grow_spraying_compliance` | Chemical label registry — provides PHI/REI for safety interval calculation |
...
1. For each `grow_spraying_input` row, look up the linked `grow_spraying_compliance` record
...
- Multiple chemicals can be applied in a single spraying event — each gets its own `grow_spraying_input` row.
- Multiple pieces of equipment can be used — each gets its own `grow_spraying_equipment` row with independent water quantities.
