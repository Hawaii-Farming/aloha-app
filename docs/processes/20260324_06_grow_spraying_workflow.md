...
| `grow_spray_seeding` | Join table — which seeding batches were treated |
...
3. Link the seeding batches being treated via `grow_spray_seeding` (one row per batch) — only batches with status `transplanted` or `harvesting` are available
