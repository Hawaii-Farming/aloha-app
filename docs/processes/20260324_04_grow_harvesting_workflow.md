...
This document describes the harvesting activity flow using `ops_task_tracker` as the activity header and `grow_harvesting` as the domain-specific record.
...
| `grow_harvesting` | Harvesting-specific data — seeding link, grade, harvest date |
| `grow_harvesting_weight` | Individual weigh-ins per container type |
| `grow_harvest_container` | Container definitions with tare weight (per variety/grade) |
| `grow_seeding` | Source batch being harvested (traceability link) |
...
- Harvest totals (total gross, total net, total containers) are derived by summing across `grow_harvesting_weight` rows. No totals are stored on the header.
- The `grow_harvesting` header is required because it carries the seeding link and grade assignment — traceability and grading are harvest-specific fields that do not exist on `ops_task_tracker`.
...
```mermaid
flowchart TD
    A[Create ops_task_tracker\nTask = Harvesting] --> B[Create grow_harvesting record]
    B --> C[Select grow_seeding batch\nfor traceability]
    C --> D[Assign grow_grade]
    D --> E[Enter harvest date]
    E --> F[Add weigh-in: grow_harvesting_weight]
